...
def transfer_data_gpu(ncells: int, src_nlon: int, tgt_nlon: int):
    ...
    src_i = np.zeros((ncells), dtype=np.int32)
    src_j = np.zeros((ncells), dtype=np.int32)
    tgt_i = np.zeros((ncells), dtype=np.int32)
    tgt_j = np.zeros((ncells), dtype=np.int32)
    xarea = np.zeros((ncells), dtype=np.float64)    

    create_xgrid_transfer_data(c_int(ncells), c_int(src_nlon), c_int(tgt_nlon),
...
def get_2dx2d_order1_gpu(src_nlon: int,
    ...
    ncells = create_xgrid_order1_gpu_wrapper(c_int(src_nlon), c_int(src_nlat),
                                              c_int(tgt_nlon), c_int(tgt_nlat),
                                              src_lon, src_lat, tgt_lon, tgt_lat,
                                              src_mask, tgt_mask)
    
    return transfer_data_gpu(ncells, src_nlon, tgt_nlon)
