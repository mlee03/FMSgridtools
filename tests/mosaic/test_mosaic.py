...
def make_grid(gridfile):
    ...
    area = xr.DataArray(
        np.ones((grid_size, grid_size), dtype=np.float64), dims=["ny", "nx"]
    )
...
def test_create_regional_input():
    ...
    yt_data = xr.DataArray(data=yt, dims=["grid_yt_sub01"]).astype(np.float64)

    xr.Dataset(
        data_vars={"grid_xt_sub01": xt_data, "grid_yt_sub01": yt_data}
    ).to_netcdf(f"regional_input_file.tile{tile_number}.nc")
